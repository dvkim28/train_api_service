...
def get_route_retrieve_url(route_id):
    return reverse("train_station:route-detail",
                   args=[route_id])


def get_station_retrieve_url(station_id):
    return reverse("train_station:station-detail",
                   args=[station_id])
...
class AuthorizedRoutesAPITestCase(APITestCase):
    ...
    def test_station_create(self):
        response = self.client.post(STATIONS_URL, {
            "name": "test station",
            "latitude": 200,
            "longitude": 200,
        })
